...
if (Meteor.isClient) {
  ...
  Template.main.events({
    ...
    'click #submit_btn': function () { // Clicking submit to get recipes
      Meteor.call('getRecipe', Template.ingr.foodItems, function(error, response) {
        if (response) {
          var recipes = JSON.parse(response.content);
          Session.set('search_results', recipes); // Dictionary of recipes
  ...
  Template.searchResults.helpers({
    results: function () {
      console.log(Session.get('search_results').matches);
      return Session.get("search_results").matches;
    }
  });
}
  ...
  Meteor.methods({
    ...
    getRecipe: function (ingredients) {
      ...
      try {
        ...
        // var results =  HTTP.get("http://api.yummly.com/v1/api/recipes?_app_key=f417e2ff0264769fc557b116fb4197d0&_app_id=b1229d17&allowedIngredient[]=cognac");
        // console.log(results);
        //result = 'test'
        return results;
      } catch (e) {
